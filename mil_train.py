...
class FiLM(nn.Module):
    def __init__(self, clinic_dim, feat_dim):
        super().__init__()
        self.fc = nn.Linear(clinic_dim, feat_dim * 2)

    def forward(self, z_img, clinic):
        gamma, beta = self.fc(clinic).chunk(2, dim=-1)
        return gamma * z_img + beta


class FiLM(nn.Module):
    def __init__(self, clinic_dim, feat_dim, hidden_dim=128):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(clinic_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, feat_dim * 2)
        )

    def forward(self, z_img, clinic):
        gamma, beta = self.net(clinic).chunk(2, dim=-1)
        gamma = 1 + gamma   # important stabilization trick
        return gamma * z_img + beta
...
class PatientMILFiLM(nn.Module):
    def __init__(
        self,
        in_dim=768,
        clinic_dim=213,
        ...
        if architecture == "attention":
            ...
            self.pool = TransformerMILPool(in_dim, d_hidden=256)
            feat_dim = 256
        else:
            raise ValueError
        self.film = FiLM(clinic_dim, feat_dim)
        self.dropout = nn.Dropout(dropout)
        self.classifier = nn.Sequential(
            nn.Linear(feat_dim, feat_dim // 2),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(feat_dim // 2, n_labels)
        )

        #self.classifier = nn.Linear(feat_dim, n_labels)
        if freeze_mil:
            for p in self.pool.parameters():
                p.requires_grad = False
    
    def forward(self, feats, clinic):
        z_img, attn = self.pool(feats)
        z_fused = self.film(z_img, clinic)
        #z_fused = self.dropout(z_fused)
        logits = self.classifier(z_fused)
        return logits, z_fused, attn
