...
class PatientMultimodalDataset(Dataset):
    ...
    def _select_instances(self, feats, pid):
        ...
        if self.max_feats <= 0 or self.instance_strategy == "all":
            return feats

        if N < self.max_feats:
            pad = torch.zeros(self.max_feats - N, feats.shape[1])
        ...
        if self.instance_strategy == "random":
            ...
            g = torch.Generator().manual_seed(int(pid) % (2**32))
            idxs = torch.randperm(N, generator=g)[:self.max_feats]
    ...
    def __getitem__(self, idx):
        ...
        feats = torch.load(
            os.path.join(self.feat_dir, f"{pid}.pt"),
            weights_only=True
        )["feats"]

        feats = self._select_instances(feats, pid)

        clinic = torch.tensor(self.clinic_dict[pid], dtype=torch.float32)
        label = torch.tensor(self.labels[idx])

        return feats, clinic, label, pid
